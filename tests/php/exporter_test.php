...
function propType(DOMXPath $xp, string $id): string {
    return trim((string)$xp->evaluate('string(//Товар/ЗначенияСвойств/ЗначенияСвойства[Ид="' . $id . '"]/Тип)'));
}
...
foreach (['CML2_ACTIVE','CML2_CODE','CML2_SORT','CML2_ACTIVE_FROM','CML2_ACTIVE_TO','CML2_PREVIEW_TEXT','CML2_DETAIL_TEXT','CML2_PREVIEW_PICTURE'] as $id) {
    assertTrue($xp->query('//Классификатор/Свойства/Свойство[Ид="' . $id . '"]')->length === 1, 'classifier standard field ' . $id . ' missing');
}
assertTrue($xp->query('//Классификатор/Свойства/Свойство[Ид="CML2_CODE" and Наименование="Символьный код"]')->length === 1, 'CML2_CODE classifier definition failed');
assertTrue($xp->query('//Классификатор/Свойства/Свойство[Ид="CML2_CODE"]/БитриксКод')->length === 0, 'standard fields must not contain БитриксКод');
assertTrue($xp->query('//Классификатор/Свойства/Свойство[Ид="CML2_CODE"]/БитриксТипСвойства')->length === 0, 'standard fields must not contain БитриксТипСвойства');
assertTrue($xp->query('//Классификатор/Свойства/Свойство[Ид="CML2_CODE"]/СсылочныйИнфоблок')->length === 0, 'standard fields must not contain СсылочныйИнфоблок');
...
assertTrue(propValues($xp,'CML2_CODE') === ['testovaya-statya'], 'CML2_CODE standard value failed');
assertTrue($xp->query('//Товар/ЗначениеРеквизита[Наименование="CML2_CODE"]')->length === 0, 'CML2_CODE must not be exported as requisite');
assertTrue(propValues($xp,'CML2_ACTIVE') === ['false'], 'CML2_ACTIVE standard value failed');
assertTrue(propValues($xp,'CML2_SORT') === ['500'], 'CML2_SORT standard value failed');
assertTrue(propValues($xp,'CML2_ACTIVE_FROM') === [''], 'CML2_ACTIVE_FROM empty value must be present');
assertTrue(propValues($xp,'CML2_ACTIVE_TO') === [''], 'CML2_ACTIVE_TO empty value must be present');
assertTrue(propValues($xp,'CML2_PREVIEW_PICTURE') === [''], 'CML2_PREVIEW_PICTURE empty value must be present');
assertTrue(propType($xp,'CML2_PREVIEW_TEXT') === 'text', 'CML2_PREVIEW_TEXT type failed');
assertTrue(propType($xp,'CML2_DETAIL_TEXT') === 'html', 'CML2_DETAIL_TEXT type failed');
...
$brokenDoc = (new BitrixArticleXmlExporter())->export(basePayload());
$brokenXp = new DOMXPath($brokenDoc);
$codeNode = $brokenXp->query('//Товар/ЗначенияСвойств/ЗначенияСвойства[Ид="CML2_CODE"]')->item(0);
assertTrue($codeNode instanceof DOMElement, 'broken test CML2_CODE node missing');
$codeNode->parentNode?->removeChild($codeNode);
$product = $brokenXp->query('//Товар')->item(0);
assertTrue($product instanceof DOMElement, 'broken test product missing');
$requisite = $brokenDoc->createElement('ЗначениеРеквизита');
$requisite->appendChild($brokenDoc->createElement('Наименование', 'CML2_CODE'));
$requisite->appendChild($brokenDoc->createElement('Значение', 'testovaya-statya'));
$product->appendChild($requisite);
assertTrue((new BitrixArticleXmlExporter())->validate($brokenDoc) !== [], 'validate() must reject CML2_CODE in requisite');
