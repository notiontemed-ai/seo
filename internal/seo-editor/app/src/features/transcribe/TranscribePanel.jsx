...
export default function TranscribePanel({ embedded = false, onApplied } = {}) {
  const {
    article, patchArticle, setBlocks, setNotice,
    caseResult, setCaseResult, setTopicRisk,
  } = useStore();
    ...
    try {
      ...
      const result = {
        ...
        topics: (Array.isArray(data.topics) ? data.topics : []).map((t) => ({
          ...
          secondary_queries: Array.isArray(t.secondary_queries)
            ? t.secondary_queries.map((s) => String(s).trim()).filter(Boolean)
            : [],
          fit: t.fit || '',
          risk: null,
        })),
  ...
  const chooseTopic = (topic) => {
    patchArticle(topicToArticlePatch(topic, caseResult));
    setBlocks([...article.blocks, caseStudyBlock(caseResult)]);
    if (onApplied) onApplied(['name', 'code', 'primary_query', 'secondary_queries']);
    setNotice({ type: 'success', text: 'Тема применена: поля задачи заполнены, кейс добавлен блоком «Случай из практики».' });
  };

  return (
    <div className={embedded ? 'transcribe-embedded' : 'panel'}>
      {!embedded && <h3 className="panel-title">Транскрибация кейса</h3>}
            ...
            <div className="case-topics">
              ...
              {caseResult.topics.map((topic, i) => (
                <div className="case-topic" key={i}>
                  ...
                  <div className="muted">Primary: {topic.primary_query || '—'}</div>
                  {topic.secondary_queries?.length > 0 && (
                    <div className="chips case-topic-secondary">
                      {topic.secondary_queries.map((q, k) => (
                        <span className="chip" key={k}>{q}</span>
                      ))}
                    </div>
                  )}
