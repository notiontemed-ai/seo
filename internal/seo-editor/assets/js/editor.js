...
async function startExternalUniqueness(button){setButtonBusy(button,true);try{const text=collectExternalUniquenessTextSync();if(text.length<100)throw new Error('Для проверки TEXT.RU требуется не менее 100 символов.');const content_hash=await calculateExternalUniquenessHash();const saved=JSON.parse(sessionStorage.getItem('temed_external_uniqueness')||'null');if(saved&&saved.text_uid&&saved.content_hash===content_hash&&saved.status==='processing'){APP_STATE.uniqueness.external=saved;renderExternalUniqueness();scheduleExternalUniquenessPoll();return;}APP_STATE.uniqueness.external={status:'queued',content_hash,started_at:new Date().toISOString(),attempt:0};renderExternalUniqueness();const payload=await callN8n('start_external_uniqueness',{text,content_hash},button);const data=payload.data||{};APP_STATE.uniqueness.external={...data,attempt:0,started_at:new Date().toISOString(),status:data.status||'processing'};sessionStorage.setItem('temed_external_uniqueness',JSON.stringify(APP_STATE.uniqueness.external));renderExternalUniqueness();scheduleExternalUniquenessPoll();}catch(e){APP_STATE.uniqueness.external={status:'failed',message:String(e.message||e)};renderExternalUniqueness();}finally{setButtonBusy(button,false);}}
...
function scheduleExternalUniquenessPoll(){stopExternalUniquenessPoll();const state=APP_STATE.uniqueness.external;if(!state||!state.text_uid||state.status!=='processing')return;const attempt=Number(state.attempt||0);if(attempt>=40){APP_STATE.uniqueness.external={...state,status:'failed',message:'Проверка TEXT.RU всё ещё выполняется. Результат можно запросить повторно позднее.'};sessionStorage.setItem('temed_external_uniqueness',JSON.stringify(APP_STATE.uniqueness.external));renderExternalUniqueness();return;}externalPollTimer=setTimeout(pollExternalUniqueness,attempt===0?10000:15000);}
async function pollExternalUniqueness(){const state=APP_STATE.uniqueness.external;if(!state||!state.text_uid||state.status!=='processing')return;const currentHash=await calculateExternalUniquenessHash();if(state.content_hash!==currentHash){await markExternalUniquenessOutdated();return;}try{const payload=await callN8n('get_external_uniqueness',{text_uid:state.text_uid,content_hash:state.content_hash},null);const data=payload.data||{};if(data.content_hash&&data.content_hash!==currentHash){APP_STATE.uniqueness.external={...state,...data,status:'outdated'};sessionStorage.removeItem('temed_external_uniqueness');renderExternalUniqueness();return;}const next={...state,...data,attempt:Number(state.attempt||0)+1,status:data.status||'processing'};APP_STATE.uniqueness.external=next;sessionStorage.setItem('temed_external_uniqueness',JSON.stringify(next));renderExternalUniqueness();if(next.status==='processing')scheduleExternalUniquenessPoll();else stopExternalUniquenessPoll();}catch(e){APP_STATE.uniqueness.external={...state,status:'failed',message:String(e.message||e)};renderExternalUniqueness();}}
