...
let newArticleSectionCodeTouched=false;
function transliterateSectionCode(value){const map={а:'a',б:'b',в:'v',г:'g',д:'d',е:'e',ё:'e',ж:'zh',з:'z',и:'i',й:'y',к:'k',л:'l',м:'m',н:'n',о:'o',п:'p',р:'r',с:'s',т:'t',у:'u',ф:'f',х:'h',ц:'c',ч:'ch',ш:'sh',щ:'sch',ъ:'',ы:'y',ь:'',э:'e',ю:'yu',я:'ya'};return String(value||'').trim().toLowerCase().split('').map(ch=>map[ch]??ch).join('').replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,'').replace(/-{2,}/g,'-');}
function selectedArticleSectionData(){const sectionId=String(fieldValue('article_section_id')||'').trim();if(!sectionId||sectionId==='__new__')return null;const source=APP_DATA?.dictionaries?.article_sections;const sections=Array.isArray(source)?source:(Array.isArray(source?.new)?source.new:[]);return sections.find(item=>String(item?.id??'')===sectionId)||null;}
function syncNewArticleSectionCode(){if(newArticleSectionCodeTouched)return;const code=transliterateSectionCode(fieldValue('new_article_section'));const el=document.getElementById('new_article_section_code');if(code&&el)el.value=code;}
function syncSection(){
  ...
  if(isNew)syncNewArticleSectionCode();
  document.getElementById('article_section_code').value=
    isNew?fieldValue('new_article_section_code'):(option&&option.dataset.code?option.dataset.code:'');
}
...
document.getElementById('new_article_section_code')?.addEventListener('input',()=>{newArticleSectionCodeTouched=true;syncSection();});
...
function xmlExportSectionValue(){const sectionId=fieldValue('article_section_id');return sectionId==='__new__'?fieldValue('new_article_section'):(sectionId||'');}
function bitrixXmlPayload(){const article=currentArticleObject();const formId=fieldValue('form_id').trim();const selectedSection=selectedArticleSectionData();return {...article,name:article.name,code:article.code,preview_text:article.preview_text,detail_html:article.detail_html,section:selectedSection?String(selectedSection.id):xmlExportSectionValue(),section_name:selectedSection?String(selectedSection.name||''):fieldValue('new_article_section'),section_code:selectedSection?String(selectedSection.code||''):fieldValue('new_article_section_code'),section_active:selectedSection?String(selectedSection.active||'Y'):'Y',section_sort:selectedSection?Number(selectedSection.sort||500):500,section_parent_id:selectedSection?Number(selectedSection.parent_id||0):0,primary_query:fieldValue('primary_query'),secondary_queries:fieldValue('secondary_queries'),search_intent:fieldValue('search_intent'),article_structure:fieldValue('article_structure'),article_structure_name:fieldValue('article_structure_name'),article_structure_version:fieldValue('article_structure_version'),short_answer:fieldValue('result_short_answer'),region:fieldValue('region'),region_xml_id:fieldValue('region_xml_id'),article_template:fieldValue('article_template'),article_template_xml_id:fieldValue('article_template_xml_id'),author_id:fieldValue('author_id'),medical_reviewer_id:fieldValue('medical_reviewer_id'),medical_reviewed_at:fieldValue('medical_reviewed_at'),content_updated_at:fieldValue('content_updated_at'),sources:article.sources,related_articles:getSelectedRelatedArticles(),related_services:getSelectedRelatedServices(),related_clinics:getSelectedRelatedClinics(),article_type:fieldValue('article_type'),article_type_xml_id:fieldValue('article_type_xml_id'),featured_image_alt:fieldValue('featured_image_alt'),show_form:fieldValue('show_form')||(formId?'Y':'N'),form_id:formId,form_button_text:fieldValue('form_button_text').trim()};}
function validateXmlRequiredFields(payload){const missing=Object.keys(XML_REQUIRED_FIELD_LABELS).filter(code=>code!=='form_id'&&!String(payload?.[code]??'').trim());if(String(payload?.show_form||'').trim()==='Y'&&!String(payload?.form_id||'').trim())missing.push('form_id');const sectionId=String(fieldValue('article_section_id')||'').trim();if(sectionId==='__new__'){if(!fieldValue('new_article_section_code').trim())throw new Error('Укажите символьный код нового раздела');}else if(sectionId){const selectedSection=selectedArticleSectionData();if(!selectedSection)throw new Error('Не удалось получить данные выбранного раздела из справочника. Обновите справочники и повторно выберите раздел');if(!String(selectedSection.name||'').trim())throw new Error('Не удалось получить данные выбранного раздела из справочника. Обновите справочники и повторно выберите раздел');if(!String(selectedSection.code||'').trim())throw new Error('У выбранного раздела не заполнен символьный код');}return missing;}
...
['result_name','result_code','result_detail_html','primary_query','article_type','author_search','medical_reviewer_search','article_section_id','new_article_section','new_article_section_code'].forEach(id=>{const el=document.getElementById(id);['input','change'].forEach(eventName=>el?.addEventListener(eventName,()=>{el.classList.remove('xml-field-error');}));});
...
async function openDraftSaveModal(){
  const nameField=document.getElementById('draft_save_name');
  if(nameField&&!nameField.value)nameField.value=APP_STATE.draft.name||fieldValue('task_name')||'';
  const statusField=document.getElementById('draft_save_status');
  if(statusField)statusField.value=['active','archived'].includes(APP_STATE.draft.status)?APP_STATE.draft.status:'active';
  document.getElementById('draftSaveModal')?.classList.remove('is-hidden');
  nameField?.focus();
}
async function saveCurrentDraft(button=null){
  const name=(fieldValue('draft_save_name')||fieldValue('task_name')||'').trim();
  if(!name){alert('Укажите название черновика.');document.getElementById('draft_save_name')?.focus();return;}
  const status=['active','archived'].includes(fieldValue('draft_save_status'))?fieldValue('draft_save_status'):'active';
  const snapshot=collectDraftSnapshot(); const hash=await draftSha256(snapshot);
  const metadata={name,status,save_comment:fieldValue('draft_save_comment')||''};
...
async function refreshDraftList(){const box=document.getElementById('draftList');if(!box)return;box.innerHTML='<div class="hint">Загрузка…</div>';try{const result=await callDraftApi('draft_list',{query:fieldValue('draft_search'),status:fieldValue('draft_filter_status'),limit:50,offset:0});const items=result.items||[];box.innerHTML=items.map(item=>{const id=escapeHtml(item.DRAFT_ID||'');const archived=String(item.STATUS||'')==='archived';const archiveBtn=archived?'<button type="button" class="btn" onclick="unarchiveDraft(\''+id+'\')">Вернуть из архива</button>':'<button type="button" class="btn" onclick="archiveDraft(\''+id+'\')">В архив</button>';return '<div class="draft-card"><b>'+escapeHtml(item.NAME||item.name||'Без названия')+'</b><div class="hint">v'+escapeHtml(item.CURRENT_VERSION||'')+' · '+(archived?'архив':'активный')+' · '+escapeHtml(item.UPDATED_AT||'')+'</div><div class="btnrow"><button type="button" class="btn" onclick="loadDraft(\''+id+'\')">Открыть</button>'+archiveBtn+'<button type="button" class="btn" onclick="purgeDraft(\''+id+'\')">Удалить</button></div></div>';}).join('')||'<div class="hint">Черновики не найдены</div>';}catch(e){box.innerHTML='<div class="callout warn">'+escapeHtml(String(e.message||e))+'</div>';}}
async function archiveDraft(draftId){await callDraftApi('draft_archive',{draft_id:draftId});return refreshDraftList();}
async function unarchiveDraft(draftId){await callDraftApi('draft_unarchive',{draft_id:draftId});return refreshDraftList();}
