...
async function currentUniquenessHash(){const a=collectUniquenessText();return browserHash([a.name,a.preview_text,a.short_answer,a.detail_html].join('\n\n'));}
async function markUniquenessOutdated(){const h=await currentUniquenessHash();let changed=false;['internal','external'].forEach(k=>{const r=APP_STATE.uniqueness[k];if(r&&r.status==='completed'&&r.client_hash&&r.client_hash!==h){r.status='outdated';changed=true;}});if(changed){document.getElementById('uniqueness_outdated_warning')?.classList.remove('is-hidden');renderInternalUniqueness();renderExternalUniqueness();}}
function renderUniqueness(targetId,result){const box=document.getElementById(targetId);if(!box)return;const status=result?.status||'not_started';box.dataset.status=status;const statusEl=box.querySelector('.status');if(statusEl)statusEl.textContent=status==='outdated'?'Текст изменён после проверки. Результат устарел.':status;if(result&&statusEl&&result.uniqueness_percent!==undefined){statusEl.textContent=`${status}: ${result.uniqueness_percent}% · hash ${String(result.content_hash||'').slice(0,12)} · ${result.checked_at||''}`;}const matches=box.querySelector('.matches');if(matches){matches.textContent='';(result?.matches||result?.urls||[]).forEach(item=>{const div=document.createElement('div');div.className='match-item';const link=item.url?document.createElement('a'):null;if(link){link.href=item.url;link.target='_blank';link.rel='noopener noreferrer';link.textContent=item.url;div.appendChild(link);}else{div.textContent=item.name||'Совпадение';}const pct=document.createElement('span');pct.textContent=' '+(item.matched_percent||0)+'%';div.appendChild(pct);matches.appendChild(div);});}}
...
async function runInternalUniqueness(button){setButtonBusy(button,true);try{const client_hash=await currentUniquenessHash();APP_STATE.uniqueness.internal={status:'processing',client_hash};renderInternalUniqueness();const response=await fetch('proxy.php',{method:'POST',credentials:'same-origin',cache:'no-store',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify({action:'check_internal_uniqueness',article:collectUniquenessText()})});const payload=await response.json();if(!response.ok||payload.success!==true)throw new Error(payload.message||payload.error||'Ошибка проверки');APP_STATE.uniqueness.internal={...payload.data,client_hash};document.getElementById('uniqueness_outdated_warning')?.classList.add('is-hidden');renderInternalUniqueness();logAction('Внутренняя уникальность проверена',payload.data);}catch(e){APP_STATE.uniqueness.internal={status:'failed',message:String(e.message||e)};renderInternalUniqueness();alert(String(e.message||e));}finally{setButtonBusy(button,false);}}
async function startExternalUniqueness(button){setButtonBusy(button,true);try{const article=collectUniquenessText();const text=[article.name,article.preview_text,article.short_answer,article.detail_html.replace(/<[^>]+>/g,' ')].join('\n\n').trim();if(text.length<100)throw new Error('Для TEXT.RU нужно не менее 100 символов.');const client_hash=await currentUniquenessHash();const payload=await callN8n('start_external_uniqueness',{text,article},button);APP_STATE.uniqueness.external={...(payload.data||{}),client_hash,status:(payload.data&&payload.data.status)||'processing'};sessionStorage.setItem('temed_external_uniqueness',JSON.stringify(APP_STATE.uniqueness.external));renderExternalUniqueness();pollExternalUniqueness();}catch(e){APP_STATE.uniqueness.external={status:'failed',message:String(e.message||e)};renderExternalUniqueness();}finally{setButtonBusy(button,false);}}
let externalPollTimer=null;function pollExternalUniqueness(){clearTimeout(externalPollTimer);const state=APP_STATE.uniqueness.external;if(!state||!state.provider_uid||state.status!=='processing')return;let attempts=Number(state.attempts||0);if(attempts>=40)return;externalPollTimer=setTimeout(async()=>{try{const payload=await callN8n('get_external_uniqueness',{uid:state.provider_uid},null);const data=payload.data||{};APP_STATE.uniqueness.external={...state,...data,attempts:attempts+1,status:data.status||'processing'};sessionStorage.setItem('temed_external_uniqueness',JSON.stringify(APP_STATE.uniqueness.external));renderExternalUniqueness();if(APP_STATE.uniqueness.external.status==='processing')pollExternalUniqueness();}catch(e){APP_STATE.uniqueness.external={...state,status:'failed',message:String(e.message||e)};renderExternalUniqueness();}},attempts===0?10000:15000);}
...
async function sendAssistantMessage(){const input=document.getElementById('assistant_input');const message=input?.value.trim();if(!message)return;APP_STATE.assistant.messages.push({role:'user',content:message});APP_STATE.assistant.messages=APP_STATE.assistant.messages.slice(-20);renderAssistantMessage('user',message);input.value='';try{const payload=await callN8n('assistant_chat',collectAssistantContext(),document.getElementById('assistant_send'));const data=payload.data||{};const answer=String(data.answer||'Ассистент не вернул текст ответа.');APP_STATE.assistant.messages.push({role:'assistant',content:answer});APP_STATE.assistant.messages=APP_STATE.assistant.messages.slice(-20);sessionStorage.setItem('temed_assistant_'+(fieldValue('generation_id')||'draft'),JSON.stringify(APP_STATE.assistant.messages));renderAssistantMessage('assistant',answer);renderAssistantSources(data.sources);renderAssistantSuggestions(data.suggestions);}catch(e){renderAssistantMessage('assistant','Ошибка ассистента: '+String(e.message||e));}}
async function downloadBitrixXml(button){setButtonBusy(button,true);try{const article=currentArticleObject();const payload={...article,name:article.name,preview_text:article.preview_text,detail_html:article.detail_html,section:fieldValue('article_section_id')||fieldValue('article_section'),article_structure:fieldValue('article_structure'),article_structure_name:fieldValue('article_structure_name'),article_structure_version:fieldValue('article_structure_version'),search_intent:fieldValue('search_intent'),article_type:fieldValue('article_type'),author_id:fieldValue('author_id'),medical_reviewer_id:fieldValue('medical_reviewer_id'),region:fieldValue('region'),related_articles:article.related_articles};const response=await fetch('export.php',{method:'POST',credentials:'same-origin',cache:'no-store',headers:{'Content-Type':'application/json','Accept':'application/xml, application/json'},body:JSON.stringify(payload)});const type=response.headers.get('Content-Type')||'';if(!response.ok&&type.includes('application/json')){const err=await response.json();throw new Error(err.message||'Ошибка XML');}if(!response.ok)throw new Error('Ошибка HTTP '+response.status);const blob=await response.blob();const disposition=response.headers.get('Content-Disposition')||'';const match=disposition.match(/filename="?([^";]+)"?/);const filename=match?match[1]:'bitrix-article.xml';const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=filename;document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url);setFieldValue('save_result',`XML сформирован: ${filename}\nСтатус статьи в XML: неактивна`);logAction('XML сформирован',{filename});}catch(e){setFieldValue('save_result','Не удалось сформировать XML: '+String(e.message||e));alert(String(e.message||e));}finally{setButtonBusy(button,false);}}
...
try{const saved=JSON.parse(sessionStorage.getItem('temed_external_uniqueness')||'null');if(saved){APP_STATE.uniqueness.external=saved;renderExternalUniqueness();pollExternalUniqueness();}}catch(_){ }
