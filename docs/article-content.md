...
Связка со структурами: `internal/seo-editor/data/article_structures.json` → каждый
`config` задаёт `structure` (порядок блоков с `required`/`repeat`). ИИ при генерации
получает этот список; редактор предзаполняет скелет обязательных блоков в пустом
шаге «Текст» и показывает чек-лист их готовности, затем блоки правятся вручную.
